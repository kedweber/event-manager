			...
			<? foreach ($attendees as $attendee) : ?>
				<tr>
					<td style="text-align: center;">
						<?= @helper('grid.checkbox', array('row' => $attendee))?>
					</td>
					<td>
						<a href="<?= @route('view=attendee&id='.$attendee->id); ?>">
							<?= $attendee->title; ?>
						</a>
					</td>
					<td>
						<?= @helper('grid.order', array('row' => $attendee, 'total' => $total)); ?>
					</td>
					<td>
						<?= @helper('grid.enable', array('row' => $attendee)); ?>
					</td>
				</tr>
			<? endforeach; ?>
			<? if (!count($attendees)) : ?>
